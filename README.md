...
Pages are created as Markdown files in [`Content`](Content). At its core, the site generator copies files in `Content` to `Output`, while processing Markdown files from `path/file.md` to `path/file/index.html`.

A blog post is a page with a date in the front matter. Index pages and feeds are generated for blog posts, including `micro-feed.json`, which is the data source for [@douglas](https://micro.blog/douglas) on [Micro.blog](https://micro.blog/), which cross-posts to various social media sites.
